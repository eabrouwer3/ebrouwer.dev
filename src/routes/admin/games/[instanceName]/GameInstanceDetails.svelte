<script lang="ts">
	import { manageServer, getGameServer } from "../games.remote";
	...
	let { instanceName }: Props = $props();

	const { name, game, subdomain, status, vmInstance } = await getGameServer(instanceName);
